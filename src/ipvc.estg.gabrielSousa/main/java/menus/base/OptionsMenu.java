...
public abstract class OptionsMenu implements Menu {
    ...
    public OptionsMenu(Menu[] options) {
    ...
    public final void run() {
        Stack<Menu> menusStack = new Stack<>();
        menusStack.push(this);
        while (!menusStack.empty()) {
            Menu currentMenu = menusStack.peek();
            currentMenu.action();
            if (currentMenu instanceof OptionsMenu) {
                Menu nextMenu = ((OptionsMenu) currentMenu).lastSelectedSubMenu;
                if (nextMenu == null) {
                    menusStack.pop();
                } else {
